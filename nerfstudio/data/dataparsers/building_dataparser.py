...
@dataclass
class BuildingDataParserConfig(DataParserConfig):
    """sitcoms3D dataset parser config"""

    _target: Type = field(default_factory=lambda: Building)
    """target class to instantiate"""
    data: Path = Path("data/building")
...
@dataclass
class Building(DataParser):
    """Sitcoms3D Dataset"""

    config: BuildingDataParserConfigConfig

    def _generate_dataparser_outputs(self, split="train"):
        cameras_json = load_from_json(self.config.data / "meta_data.json")
        ...
        segmentations_folder = f"semantics{downscale_suffix}"
        ...
        if self.config.include_semantics:
            ...
            panoptic_classes = load_from_json(self.config.data / "segmentation.json")
            classes = panoptic_classes["object"]
            colors = torch.tensor(panoptic_classes["object_colors"], dtype=torch.float32)
            semantics = Semantics(filenames=filenames, classes=classes, colors=colors, mask_classes=["background"])
