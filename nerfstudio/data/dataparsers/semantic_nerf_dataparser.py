...
@dataclass
class SemanticNeRFDataParserConfig(DataParserConfig):
    """SemanticNeRF dataset parser config"""
...
@dataclass
class SemanticNeRF(DataParser):
    """SemanticNeRF Dataset"""

    config: SemanticNeRFDataParserConfig
    ...
    def _generate_dataparser_outputs(self, split="train"):
        ...
        for i, frame in enumerate(meta["frames"]):
            ...
            segmentation_filenames.append(self.config.data / frame["segmentation_path"])
        ...
        semantics = Semantics(
            ...
            classes=classes,
            colors=colors,
            mask_classes=["background"],  # Use background as a mask ?
        )
    ...
    #     for frame in frames:
    #         image_filenames.append(self.config.data / frame["file_path"])
    #         segmentation_filenames.append(self.config.data / frame["segmentation_path"])
    ...
    #         classes = panoptic_classes["classes"]
    #         colors = (
    #             torch.tensor(panoptic_classes["colors"], dtype=torch.float32) / 255.0
    #         )
    #         semantics = Semantics(
    #             filenames=segmentation_filenames,
    #             classes=classes,
    #             colors=colors,
    #             # mask_classes=["person"], Use background as a mask ?
    #         )
